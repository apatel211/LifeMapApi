...
<h1 id="sql-client-unit-tests">sql-client Unit Tests</h1>
<div id=TofC>
...
<p>This directory contains automated tests for the <strong>sql-client</strong> module.</p>
<p>These tests can be executed by running:</p>
<pre><code>make test
</code></pre><p>(or <code>make clean test</code>, for good measure) from this module&#39;s root
directory.</p>
<p>A test coverage report can be generated by running:</p>
<pre><code>make coverage
</code></pre><p>(or <code>make clean coverage</code>) from that same directory.</p>
<p>While you can run this test suite without any external
(non-Node.js-related) dependencies, several of the database-specific
tests require an actual database instance to test against.</p>
<p>When you run this suite, those tests that require an external database
will be skipped if the database is not accessible.</p>
<p>(When this happens, a prominent warning is printed to STDERR,
directing you to this file.)</p>
<p>To set up the test database(s) and enable the tests, follow the
instructions below.</p>
<h2 id="postgresql">PostgreSQL</h2>
<p>To test the PostgreSQLClient classes, you&#39;ll need to create a user and
database within an accessible instance of PostgreSQL.</p>
<p>Specifically, you&#39;ll need to create the account <code>sqlclient_test_user</code>
and database <code>sqclient_test_db</code> in a <em>local</em> database instance. (More
generally, the database to connect to is specified by the
<code>CONNECT_STRING</code> variable defined in <code>test-postgresql-client.coffee</code>.)</p>
<p>Assuming you have a local instance of PostgreSQL running on the
default port, you can create this account by running:</p>
<pre><code>sudo su postgres -c &quot;psql -Upostgres -f  ./test/create-postgresql-test-db.sql&quot;
</code></pre><p>which will execute the
<a href="./create-postgresql-test-db.sql">create-postgresql-test-db.sql</a>
script.</p>
<p>Should you want to remove this test user and database, you can run:</p>
<pre><code>sudo su postgres -c &quot;psql -Upostgres -f  ./test/drop-postgresql-test-db.sql&quot;
</code></pre><p>which will execute the
<a href="./drop-postgresql-test-db.sql">drop-postgresql-test-db.sql</a>
script.</p>
<h2 id="mysql">MySQL</h2>
<p>To test the MySQLClient classes, you&#39;ll need to create a user and
database within an accessible instance of MySQL.</p>
<p>Specifically, you&#39;ll need to create the account <code>sqlclient_test_u</code>
and database <code>sqclient_test_db</code> in a local database instance. (More
generally, the database to connect to is specified by the
<code>CONNECT_OPTS</code> variable defined in <code>test-mysql-client.coffee</code>.)</p>
<p>Assuming you have a local instance of MySQL running you can
create this account by running:</p>
<pre><code>mysql -u root -p &lt; ./test/create-mysql-test-db.sql
</code></pre><p>which will execute the
<a href="./create-mysql-test-db.sql">create-mysql-test-db.sql</a>
script.</p>
<p>Should you want to remove this test user and database, you can run:</p>
<pre><code>mysql -u root -p &lt; ./test/create-mysql-test-db.sql
</code></pre><p>which will execute the
<a href="./drop-mysql-test-db.sql">drop-mysql-test-db.sql</a>
script.</p>
<h2 id="sqlite3">SQLite3</h2>
<p>The SQLite tests use a transient, in-memory database. No other
